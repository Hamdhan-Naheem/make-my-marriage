...
6. **Lifecycle and operational details:** Define archived-wedding access, dependent-record deletion, upload types/sizes and confirmation checks, S3/database failure handling, exact concurrent-refresh and stale-token response behavior, CSRF protection, domain/HTTPS, and production secret management. Authentication now uses a 15-minute access JWT, a fixed seven-day refresh session that rotation does not extend, a 24-hour single-use email-verification token, refresh-token rotation, and protected-request Session checks for prompt revocation. Logout-all remains optional until explicitly selected.
