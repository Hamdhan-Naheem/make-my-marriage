...
export function AuthCard({ children, description, notice, title }: AuthCardProps) {
  return (
    <section aria-labelledby="auth-title" className="mx-auto w-full max-w-md rounded-3xl border border-[#e8dfd8] bg-white p-6 shadow-[0_24px_56px_-32px_rgba(103,21,37,0.42)] sm:p-8">
      ...
      {notice ? <p className="mt-5 rounded-xl border border-[#e8dfd8] bg-[#fdf6f4] px-3 py-2.5 text-sm leading-5 text-[#6c3a40]" role="note">{notice}</p> : null}
      {children}
    </section>
  );
}
